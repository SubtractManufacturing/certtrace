...
vi.mock("@certtrace/library-engine", () => ({
  ...
  listAssignedMaterialIds: vi.fn(),
  listJobCustomers: vi.fn(),
  listJobs: vi.fn(),
  listJobsForMaterial: vi.fn(),
  listMaterialAttachments: vi.fn(),
  listMaterials: vi.fn(),
  listMaterialsForJob: vi.fn(),
  openLibrary: vi.fn(),
  removeJob: vi.fn(),
...
describe("library-client", () => {
  ...
  it("delegates confirmed options to the library engine", async () => {
    ...
    const result = {
      ...
      fieldSchema: { version: 4, fields: [], identifierKinds: [], attachmentKinds: [] },
    } as never;
