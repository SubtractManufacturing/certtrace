...
**Printer**:
A shop-named destination on this computer that refers to one printer already set up in the operating system. A Label Template may have one Printer on this computer; each OS printer is at most one Printer, and each Printer name is unique on this computer.
_Avoid_: Alias (as its own record), device, print queue (when meaning this entry)
