...
function isWindowsMissingPathMessage(message: string): boolean {
  ...
  if (lower.includes("os error 3")) {
    return true;
  }

  if (lower.includes("cannot find the path specified")) {
    return true;
  }

  const tauriFs =
    lower.includes("failed to open file") || lower.includes("failed to read directory");
  const missing = lower.includes("cannot find the path") || lower.includes("os error 3");
  return tauriFs && missing;
}

/** True when a filesystem error indicates a missing path (Node ENOENT, Tauri NotFound, Windows os error 3). */
